...
class DNSManager {
   ...
   * 设置可靠的DNS服务器
   */
  private async setReliableDNS(): Promise<void> {
   ...
   * 测试并找出最快的DNS服务器
   */
  private async findFastestDNSServers(): Promise<string[]> {
    const results: { server: string; responseTime: number }[] = []
   ...
   * 验证DNS健康状态
   */
  private async verifyDNSHealth(): Promise<boolean> {
   ...
   * 解析特定域名（带重试机制）
   */
  async resolveDomain(domain: string, maxRetries = 3): Promise<string[]> {
 ...
 * 应用启动时调用的初始化函数
 */
export async function initializeDNS(): Promise<void> {
 ...
 * 安全的域名解析函数
 */
export async function safeDNSResolve(domain: string): Promise<string[]> {
